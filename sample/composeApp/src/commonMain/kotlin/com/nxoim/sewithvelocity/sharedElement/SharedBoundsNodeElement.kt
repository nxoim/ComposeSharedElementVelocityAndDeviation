...
internal class SharedBoundsNode(
    internalState: SharedElementInternalState,
    ...
    var state: SharedElementInternalState = internalState
        internal set(value) {
            if (value != field) {
                // State changed!
                field = value
    ...
    private fun MeasureScope.approachPlace(placeable: Placeable): MeasureResult {
        if (!sharedElement.foundMatch) {
            ...
            return layout(w, h) {
                ...
                // updates targets on *every* measurement.
                // this helps check for visibility changes and
                // trigger animations
        ...
        val layer =
            requireNotNull(state.layer) {
                "Error: Layer is null when accessed for shared bounds/element : ${sharedElement.key}," +
                    "target: ${state.boundsAnimation.isTarget}, is attached: $isAttached"
            }
